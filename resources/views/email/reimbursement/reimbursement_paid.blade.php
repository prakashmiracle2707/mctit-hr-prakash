...
<body>
    ...
    <p>This is to confirm that the reimbursement amount of <strong>₹ {{ number_format($reimbursement->amount, 2) }}</strong> 
        ...
        via <strong>{{ $reimbursement->payment_type == 'UPI'  ? 'UPI': 'cash/online payment'}}</strong> on <strong>{{ $reimbursement->paid_at ? $reimbursement->paid_at->format('Y-m-d h:i A') : 'Not Paid' }}</strong>.
    </p>
